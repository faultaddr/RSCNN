...
class RSCNN_MSN(nn.Module):
    ...
    def forward(self, pointcloud: torch.cuda.FloatTensor, cls):
        ...
        context,features_context=self.context_prior(l_features[0])
        return self.FC_layer(features_context).transpose(1, 2).contiguous(),context
