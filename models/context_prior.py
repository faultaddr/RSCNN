...
class get_loss(torch.nn.Module):
    ...
    def forward(self, pred, target, trans_feat,context,context_gt):
        #print(context_gt.shape)
        print(pred.shape,target.shape,context.shape,context_gt.shape)
        context_gt = torch.bmm(context_gt,context_gt.permute(0, 2, 1))
        loss = F.cross_entropy(pred+1e-8, target)
        print('------')
        #print(context,context_gt)
        #print(pred.shape,target.shape)
        print(context_gt.shape)
        print('cr',loss)
        #loss=F.nll_loss(pred,target)
        loss_p=F.binary_cross_entropy_with_logits(context,context_gt)
        print('single cr',loss_p)
        ...
        print('loss_g_p',torch.mean(loss_g_p))
        print('loss_g_r',torch.mean(loss_g_r))
        print('loss_g_s',torch.mean(loss_g_s))
        print('loss_g',loss_g)
